...
$(function () {
    ...
    $(".linkins").live("click", function () {
        ...
        var subOdr = tr.attr("id").split("_")[0].substring(7);
        var subName = _sysModel[0].subtbls[subOdr].tablename;
        var rowId = _fnSubAdd(subName, tr.attr("id"));
    });
    ...
    $("table.subtbl").not(".readtbl").tableDnD({
        ...
        onDrop: function (table, row) {
            var rowId = row.id;
            var subOdr = rowId.split("_")[0].substring(7);
            _setSubAutoSn(subOdr);
        }
    });
...
function _afterMultiRowsSel(retctl, multiRow, subName) {
    if (arguments.length == 2) {
        ...
        for (var i = 1; i < multiRow.length; i++) {
            var newId = _fnSubAdd(subT.tablename);
...
function _genFieldsMap() {
    if (!!_sysModel[0].fieldsMap) {
        return;
    }
    var fieldArr = _sysModel[0].fields;
    _sysModel[0].fieldsMap = {};
    for (var i = 0; i < fieldArr.length; i++) {
        _sysModel[0].fieldsMap[fieldArr[i].name] = fieldArr[i];
    }
}
...
function _getFieldOdr(fldName) {
    ...
    if (_sysModel[0].fieldsMap[fldName]) {
        return _sysModel[0].fieldsMap[fldName].order;
    }
    else {
        return -1;
    }
...
function _getField(fldName) {
    if (arguments.length == 1) {
        ...
        fldName = arguments[1];
        var subT = _getSubModel(tblName);
        for (var i = 0; i < subT.fields.length; i++) {
            if (subT.fields[i].name == fldName)
...
function _getFieldById(ctlId) {
    if (ctlId.indexOf("SubTbl") == 0) {
        ...
        var fldList = _sysModel[0].fields;
...
function _formSynchData() {
    ...
    for (var mainOrd = 0; mainOrd < _sysModel.length; mainOrd++) {
        ...
        for (var j = 0; j < subTblCount; j++) {
            ...
            var dataRows = _xmlData.find("Table[TableName='" + subTbl.tablename + "']>row");
            var rowCount = dataRows.length;
            if (subTbl.notnull == "是" && rowCount == 0) {
                alert("明细表【" + subTbl.tblnamecn + "】必填，请录入数据");
                return false;
            }
            var trRows = $("tr.dataRow", "#" + subTbl.tablename);
            var trCount = trRows.length;
            for (var n = 0; n < trCount; n++) {
                var trNode = trRows.eq(n);
                var rowId = trNode.attr("id");
                var prefix = "#" + rowId;
                var rowNode = _xmlData.find(prefix);
                var oldOdr = rowNode.attr("order");
                rowNode.attr("newodr", n);
...
function _createXml(subOdr, rowId) {
    ...
    var atNode = _xmlData[0].createElement("_AutoID");
    atNode.appendChild(_xmlData[0].createCDATASection(""));
...
function _getSubId(rowId) {
    var rowNode = _xmlData.find("#" + rowId);
    var xmlEl = rowNode.find("_AutoID");
...
function _getSubOdr(subName,msg) {
    for (var i = 0; i < _sysModel[0].subtbls.length; i++) {
        var k = _sysModel[0].subtbls[i].tablename;
        if (subName == k) {
            return i;
        }
    }
...
//子表模型
function _getSubModel(subName) {
    for (var i = 0; i < _sysModel[0].subtbls.length; i++) {
        var k = _sysModel[0].subtbls[i].tablename;
        if (subName == k) {
            return _sysModel[0].subtbls[i];
        }
    }
    return null;
}
    ...
    if (typeof (snFieldOdr) == "undefined") {
        ...
        for (var i = 0; i < _sysModel[0].subtbls[subOdr].fields.length; i++) {
            ...
            if (field.dispstyle == "003") {
                _sysModel[0].subtbls[subOdr].snfield = field.name;
                snFieldOdr = field.name;
//                var rowId = "SubTbl0" + subOdr + "_srkjdslABHSAS_" + snFieldOdr;
//                $("#" + rowId).addClass("SubAutoSn");
                break;
            }
        }
    }
...
function _fnSubAdd(subName, retRowId, dataRow) {
    ...
    if (subOdr == -1)
        return;
    var model = _getSubModel(subName);
    if (model.mode == "1") {
        _fnPopAdd(subName);
        return;
    }
...
function _fnSubDelAll(subName) {
    ...
    if (subOdr == -1) {
        ...
        for (var i = 0; i <= maxOdr; i++) {
            var rowId = "SubTbl0" + subOdr + "_" + i + "_";
            _fnSubDel(subName, rowId);
        }
...
function _fnNewRowId(subName) {
    ...
    if (subOdr == -1)
        return;
    var newOdr = _sysModel[0].subtbls[subOdr].maxorder++;
    var rowId = "SubTbl0" + subOdr + "_" + newOdr + "_";
    return rowId;
}
...
function _getSubFieldOdr(subOdr, fieldName) {
    ...
    for (var i = 0; i < _sysModel[0].subtbls[subOdr].fields.length; i++) {
        var el = _sysModel[0].subtbls[subOdr].fields[i];
...
function _computeSubExp(subName, subField, rowOdr, exp) {
    ...
    if (matches != null) {
        ...
        jQuery("#SubTbl0" + subOdr + "_" + rowOdr + "_" + tOdr).val(_fixNum(eval(exp), xsLen)).change();
    }
}
